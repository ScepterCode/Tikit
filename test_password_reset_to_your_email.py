"""
Test Password Reset to scepterboss@gmail.com
"""
import os
...
print("\n" + "="*60)
print("🧪 PASSWORD RESET TEST - YOUR EMAIL")
print("="*60)

# Step 1: Check if user exists, if not create one
print(f"\n1️⃣ Checking for user: {YOUR_EMAIL}")
try:
    user_result = supabase.table('users').select('*').eq('email', YOUR_EMAIL).execute()
    
    if user_result.data:
        user = user_result.data[0]
        print(f"✅ User found: {user.get('first_name', 'User')}")
    else:
        print(f"⚠️ User not found, creating test user...")
        # Create test user
                        ...
                        <tr>
                            <td style="padding: 0 40px 40px;">
                                ...
                                <p style="color: #6b7280; font-size: 16px; line-height: 1.6; margin: 0 0 30px;">
                                    We received a request to reset your Grooovy password. Click the button below to create a new password:
                                </p>
...
try:
    ...
    if response.status_code == 200:
        ...
        if result['summary']['sent'] > 0:
            print(f"\n🎉 SUCCESS! Check {YOUR_EMAIL} for password reset email!")
            print(f"\n🔗 Reset link format:")
